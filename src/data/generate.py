...
def generate_locations(n_locations):
    ...
    lats = l_lat + np.round(
       np.random.rand(n_locations-1)*2*offset_lat - offset_lat,
       6
    )
    lons = l_lon + np.round(
       np.random.rand(n_locations-1)*2*offset_lon - offset_lon,
       6
    )
...
def generate_instance(n_locations, n_vehicle):
  ...
  model, x = vehicle_routing_model(distance_matrix=distance_matrix, n_vehicle=n_vehicle, capacity=capacity, demand=demand)
...
test_dataset_models, test_dataset_variables, test_dataset_coordinates = generate_instance_dataset(
   n_instances=3,
   n_locations=10,
   n_vehicle=3
)